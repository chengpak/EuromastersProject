// This code reads an input file and outputs the x,y and z position of entries on an event // by event basis. Each event is plotted separatedly on the same canvas.
// Right now, the input file has 10 entries so the canvas is divided (5,2). 
// You can configure this number for a different number of entries.
//
// The code is run as $ root -l -x 'OD3DEvtByEvt.C("OD.root")' where OD.root refers to the
...
void OD3DEvtByEvt(char *filename=NULL) {
 ...
 TCanvas *c1;
   ...
   c1 = new TCanvas("ntuple","ntuple", 1200,600);
   c1->Divide(5,2);
