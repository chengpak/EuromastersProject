...
void ODClusterJan30(char *filename=NULL) {
  ...
  Double_t totalqincluster = 0.0;
   ...
   for(long unsigned int iEntry = 4; iEntry <=4; iEntry++){
     ...
     // Point to event iEntry inside WCSimTree
       ...
       for(long unsigned int iTrig = 0; iTrig < 1; iTrig++){
   ...
   for ( Long64_t ient = 0; ient < ntuple -> GetEntries(); ient++ ) {
     ...
     ntuple->GetEntry(ient);
      ...
      if ((pX>maxpmtX-710.2) && (pX<maxpmtX+710.2) && (pY>maxpmtY-710.2) && (pY<maxpmtY+710.2) && (pZ>maxpmtZ-599.7) && (pZ<maxpmtZ+599.7 ) && (Q>(0.05*max1Charge) ) ){
	...
	totalqincluster = totalqincluster+Q;
      ...
      else if ((pX>maxpmtX-710.2) && (pX<maxpmtX+710.2) && (pY>maxpmtY-710.2) && (pY<maxpmtY+710.2) && (pZ>maxpmtZ-599.7)&& (pZ<maxpmtZ+599.7 ) && (Q<(0.05*max1Charge) ) ){                       
	   continue;
 ...
 endClusteringAlgorithm:    //*******  if clusters less than 3 pmt hits, clustering algorithm terminates and gets sent here.   *********
   ...
   */
   ...
   maxchargentuple->Draw("pmtZ:pmtY:pmtX");
  ...
  /*
   TCanvas *c2;
   c2 = new TCanvas("ntuple2","ntuple2", 800,600);
