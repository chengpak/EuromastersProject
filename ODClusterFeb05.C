...
void ODClusterFeb05(char *filename=NULL) {
  ...
  // ******************  Clustering algorithm  *********************************
   ...
   for ( Long64_t ient = 0; ient < ntuple -> GetEntries(); ient++ ) {
     ...
     ntuple->GetEntry(ient);
      ...
      if ((pX>maxpmtX-710.2) && (pX<maxpmtX+710.2) && (pY>maxpmtY-710.2) && (pY<maxpmtY+710.2) && (pZ>maxpmtZ-559.7) && (pZ<maxpmtZ+5959.7 ) && (Q>(0.05*max1Charge) ) ){
	...
	totalqincluster = totalqincluster+Q;
      ...
      else if ((pX>maxpmtX-710.2) && (pX<maxpmtX+710.2) && (pY>maxpmtY-710.2) && (pY<maxpmtY+710.2) && (pZ>maxpmtZ-559.7)&& (pZ<maxpmtZ+559.7 ) && (Q<(0.05*max1Charge) ) ){                       
	   continue;
   ...
   for ( Long64_t ient = 0; ient < n2ntuple -> GetEntries(); ient++ ) {
     ...
     if ((pX>maxpmtX-710.2) && (pX<maxpmtX+710.2) && (pY>maxpmtY-710.2) && (pY<maxpmtY+710.2) && (pZ>maxpmtZ-559.7) && (pZ<maxpmtZ+559.7 ) && (Q>(0.05*max1Charge) ) ){
...
endClusteringAlgorithm:    //*******  if clusters less than 3 pmt hits, clustering algorithm terminates and gets sent here.   *********
    ...
    ntuple->Draw("pmtZ:pmtY:pmtX:charge");
   ...
   maxchargentuple->SetMarkerColor(2);  // red
 
   maxchargentuple->Draw("pmtZ:pmtY:pmtX"," ", "same");


   maxcharge2ntuple->SetMarkerStyle(21);
   maxcharge2ntuple->SetMarkerColor(4); //blue

   maxcharge2ntuple->Draw("pmtZ:pmtY:pmtX","", "same");


   
   maxcharge3ntuple->SetMarkerStyle(21);
   maxcharge3ntuple->SetMarkerColor(6); // pink

   maxcharge3ntuple->Draw("pmtZ:pmtY:pmtX","", "same");
   
   
   maxcharge4ntuple->SetMarkerStyle(21);
   maxcharge4ntuple->SetMarkerColor(7);  //light blue

   maxcharge4ntuple->Draw("pmtZ:pmtY:pmtX","", "same");


   
   maxcharge5ntuple->SetMarkerStyle(21);
   maxcharge5ntuple->SetMarkerColor(8);  //green

   maxcharge5ntuple->Draw("pmtZ:pmtY:pmtX","", "same");


   maxcharge6ntuple->SetMarkerStyle(21);
   maxcharge6ntuple->SetMarkerColor(9);  //green

   maxcharge6ntuple->Draw("pmtZ:pmtY:pmtX","", "same");
   */
